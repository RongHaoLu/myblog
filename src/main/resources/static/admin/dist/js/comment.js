$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '状态', name: 'comment_sta', index: 'comment_sta', width: 60, formatter: statusFormatter},
            {label: '回复内容', name: 'meComment.content', index: 'meComment.content', width: 120},
        ],
        height: 700,
        rowNum: 10,
        rowList: [10, 20, 50],
...
function reply() {
    ...
    console.log(rowData);
    if (rowData.comment_sta.indexOf('待审核') > -1) {
        swal("请先审核该评论再进行回复!", {
            icon: "warning",
        });
        return;
...
$('#saveButton').click(function () {
    ...
    if (!validCN_ENString2_100(replyBody)) {
        ...
        var url = '/rh/comments/reply';
        var id = getSelectedRow();
        var params = {"id": id, "meComment": replyBody}
        $.ajax({
            type: 'POST',//方法类型
            url: url,
            data: params,
